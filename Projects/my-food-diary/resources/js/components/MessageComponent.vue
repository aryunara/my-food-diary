<template>
    <div id="container">
        <aside>
            ...
            <ul>
                <li v-for="friend in friends" :key="friend.id">
                    <a :href="'/dialog/' + friend.id" @click.prevent="getDialog(friend.id)">
                        <div>
        ...
        <main v-if="currentFriend">
            <header>
                <div>
                    <h2>Chat with {{ currentFriend.username }}</h2>
                </div>
            </header>
...
export default {
    ...
    methods: {
        getDialog(friendId) {
            axios.get("/dialog/" + friendId)
                .then(response => {
                    this.dialog = response.data;
                    this.currentFriend = this.friends.find(friend => friend.id === friendId);
                })
                .catch(error => {
